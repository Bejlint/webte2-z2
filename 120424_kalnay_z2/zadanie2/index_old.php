...
<script>
    async function fetchLaureates() {
        try {
            const response = await fetch('/zadanie2/api/v0/laureates');
            ...
            data.forEach(laureate => {
                ...
                laureateDiv.textContent = `Name: ${laureate.fullname ?? laureate.organisation}, Gender: ${laureate.sex ?? "ORG"}, Birth: ${laureate.birth ?? "Not known"}, Death: ${laureate.death ?? "Still alive"} `;
